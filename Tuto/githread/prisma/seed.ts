...
const main = async () => {
  ...
  for (let i = 0; i < 200; i++) {
    const ramdomUserIndex = faker.number.int({ min: 0, max: users.length - 1 })
    const ramdomPostIndex = faker.number.int({ min: 0, max: posts.length - 1 })

    const like = {
      postId: posts[ramdomPostIndex].id,
      userId: users[ramdomUserIndex].id
